...
class SnakeGame:
    ...
    # If the head hits itself
        if self.head in self.snake[1:]:
            return True

    # If the head hits the boundary
        if self.head.x >= self.w:
            self.head = Point(0, self.head.y)  # Move to the left edge
        elif self.head.x < 0:
            self.head = Point(self.w - BLOCK_SIZE, self.head.y)  # Move to the right edge
        elif self.head.y >= self.h:
            self.head = Point(self.head.x, 0)  # Move to the top edge
        elif self.head.y < 0:
            self.head = Point(self.head.x, self.h - BLOCK_SIZE)  # Move to the bottom edge
